import FrameTemplate from './Models/Frames';
...
function extractFrameData(data) {
	...
	const goalFrames = goals.map(frame => FrameTemplate.Goal(frame));
	const demoFrames = demos.map(frame => FrameTemplate.Demo(frame));
	const hitFrames = hits.map(frame => FrameTemplate.Hit(frame));
	const bumpFrames = bumps.map(frame => FrameTemplate.Bump(frame));
	const carryFrames = carries.map(frame => FrameTemplate.BallCarry(frame));
	const kickoffFrames = kickoffs.map(frame => FrameTemplate.Kickoff(frame));
